...
3. Planner 生成严格 `CreationPlan`；每个步骤声明唯一 `tool`、输入 `target_refs` 和文件变更的唯一 `output_ref`。
...
7. Writer/Reviser 的长文本通过正文内容通道生成，再以计划锁定的 `output_ref` 调用 Tool Registry；知识角色只写候选 `KnowledgeOperation`。
