<template>
  <Teleport to="body">
    <div
      v-if="visible"
      class="file-mention-popup"
      :style="{ bottom: position.bottom + 'px', left: position.left + 'px' }"
      @mousedown.prevent
    >
      <div class="header">引用文件 <span class="hint">↑↓ 选择 · Enter 确认 · Esc 取消</span></div>
      <ul>
        <li
          v-for="(item, idx) in filtered"
          :key="item.path"
          :class="{ active: idx === activeIndex }"
          @click="$emit('select', item)"
          @mouseenter="activeIndex = idx"
        >
          <FileIcon class="icon" />
          <span class="name">{{ item.name }}</span>
          <span class="path">{{ item.path }}</span>
        </li>
        <li v-if="!filtered.length" class="empty">无匹配文件</li>
      </ul>
    </div>
  </Teleport>
</template>

<script setup lang="ts">
import { computed, ref, watch } from 'vue'
import { FileText as FileIcon } from '@lucide/vue'
import type { ProjectFile } from '@/api/projectFiles'

const props = defineProps<{
  visible: boolean
  query: string
  items: ProjectFile[]
  position: { bottom: number; left: number }
}>()

const emit = defineEmits<{
  (e: 'select', file: ProjectFile): void
  (e: 'dismiss'): void
}>()

const activeIndex = ref(0)
const filtered = computed<ProjectFile[]>(() => {
  const q = props.query.toLowerCase()
  if (!q) return props.items.slice(0, 10)
  return props.items
    .filter(
      (f) =>
        f.name.toLowerCase().includes(q) ||
        f.path.toLowerCase().includes(q),
    )
    .slice(0, 10)
})

watch(() => props.query, () => { activeIndex.value = 0 })
watch(() => props.visible, (v) => { if (v) activeIndex.value = 0 })

defineExpose({
  moveUp() {
    if (!filtered.value.length) return
    activeIndex.value = (activeIndex.value - 1 + filtered.value.length) % filtered.value.length
  },
  moveDown() {
    if (!filtered.value.length) return
    activeIndex.value = (activeIndex.value + 1) % filtered.value.length
  },
  pickActive() {
    const item = filtered.value[activeIndex.value]
    if (item) emit('select', item)
  },
})
</script>

<style scoped>
.file-mention-popup {
  position: fixed;
  z-index: 9999;
  min-width: 320px;
  max-width: 480px;
  background: var(--popup-bg, #fff);
  border: 1px solid var(--border-color, #ddd);
  border-radius: 8px;
  box-shadow: 0 -4px 24px rgba(0, 0, 0, 0.12);
  padding: 6px 0;
  font-size: 13px;
}
.header {
  font-size: 11px;
  color: #888;
  padding: 4px 12px;
  display: flex;
  justify-content: space-between;
}
.header .hint {
  font-size: 10px;
  color: #aaa;
}
ul {
  list-style: none;
  margin: 0;
  padding: 0;
}
li {
  padding: 6px 12px;
  display: flex;
  align-items: center;
  gap: 8px;
  cursor: pointer;
  border-radius: 4px;
}
li.active {
  background: var(--accent-50, #f0f6ff);
}
li .icon {
  flex-shrink: 0;
  color: #888;
}
li .name {
  font-weight: 500;
  color: #333;
}
li .path {
  font-size: 11px;
  color: #999;
  white-space: nowrap;
  overflow: hidden;
  text-overflow: ellipsis;
}
li.empty {
  color: #999;
  padding: 8px 12px;
  display: block;
}
</style>
