...
def parse_provider_models(raw: Any) -> dict[str, list[str]]:
    if isinstance(raw, list):
        return {"models": _normalize_string_list(raw), "embedding_models": []}
...
def dump_provider_models(*, models: list[str], embedding_models: list[str]) -> list[str] | dict[str, list[str]]:
...
def get_provider_models(provider: AIProviderConfig, kind: Literal["chat", "embedding"]) -> list[str]:
...
def set_provider_models(provider: AIProviderConfig, kind: Literal["chat", "embedding"], models: list[str]) -> None:
    current = parse_provider_models(provider.models)
    if kind == "embedding":
        provider.models = dump_provider_models(
            models=current.get("models", []),
