    ...
    async with (
        ...
        project_response = await user.post(
            ...
            json={
                ...
                "pack_slug": "novel",
                "component_models": {
                    "operation_agent_task": f"{agent_provider_id}::{model}",
                    "operation_analyze": f"{agent_provider_id}::{model}",
                    "operation_agent_suggest": f"{agent_provider_id}::{model}",
                    "memory_llm": f"{memory_provider_id}::{model}",
                    "memory_embedding": f"{memory_provider_id}::{embedding_model}",
                    "memory_embedding_dimensions": embedding_dimensions,
                    "memory_reranker": f"{memory_provider_id}::{reranker_model}",
                },
            },
        )
